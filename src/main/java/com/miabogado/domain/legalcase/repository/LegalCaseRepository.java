...
public interface LegalCaseRepository extends JpaRepository<LegalCase, UUID> {
     ...
     */
    @Query(value = """
            select new com.miabogado.domain.legalcase.dto.CaseSummary(
                lc.id, lc.caseNumber, lc.title, lc.caseType, lc.status, lc.priority,
                c.name, u.fullName,
                (select min(d.dueDate) from CaseDeadline d
                  where d.legalCase = lc and d.status = com.miabogado.domain.legalcase.entity.DeadlineStatus.PENDING),
                lc.openedAt)
            from LegalCase lc
              join lc.client c
